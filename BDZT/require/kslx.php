...
$sql = "SELECT DISTINCT(zymc) FROM `tdd` WHERE `kslx` = '".$kslx."' AND `nf` = '".$year."'";
$rs = $db->query($sql);
...
<div class="login-container">
	...
	<?php
	   foreach ($zymcRes as $zymc) {
	       $sql = "SELECT COUNT(*) AS `num` FROM `tdd` WHERE `nf` = '".$year."' AND `kslx` = '".$kslx."' AND `zymc` = '".$zymc["zymc"]."' AND `sfbd` = '1'";
	       $rs = $db->query($sql);
	       $bdrs = $rs->fetch(PDO::FETCH_ASSOC);
	       $sql = "SELECT COUNT(*) AS `num` FROM `tdd` WHERE `nf` = '".$year."' AND `kslx` = '".$kslx."' AND `zymc` = '".$zymc["zymc"]."' AND `sfbd` = '0'";
	       $rs = $db->query($sql);
