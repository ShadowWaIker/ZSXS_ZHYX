...
if(empty($_SESSION['user'])){
    ...
    if($_POST["flag"] == "login"){
        ...
        $sql = "SELECT sfzh, tzsbh FROM tdd WHERE sfzh = '".$sfzh."' AND nf = '2018';";
        $rs = $lqcxk->query($sql);
        ...
        if($_POST["tzsbh"] === $info["tzsbh"]){
            ...
            if(empty($info["sfzh"])){
                //抽取信息
                $sql = "SELECT xh, ksh, tzsbh, xm, bj, zymc, kslx, xzh FROM tdd WHERE sfzh = '".$sfzh."';";
                $rs = $lqcxk->query($sql);
                $info1 = $rs->fetch(PDO::FETCH_ASSOC);
                $sql = "SELECT XBMC, CSNY, MZMC, ZZMMMC FROM T_TDD WHERE SFZH = '".$sfzh."';";
                $rs = $lqcxk->query($sql);
                ...
                if($success){
                    $sql = "UPDATE tdd SET ybdzt = '1' WHERE sfzh = '".$sfzh."';";
    ...
    if($_POST["flag"] == "logout"){
        ...
        $sql = "SELECT ybdzt FROM tdd WHERE sfzh = '".$sfzh."';";
        $rs = $lqcxk->query($sql);
        ...
        $sfzh = dowith_sql($_SESSION["user"]);
        $sql = "SELECT xh FROM tdd WHERE sfzh = '".$sfzh."';";
        $rs = $lqcxk->query($sql);
        $info = $rs->fetch(PDO::FETCH_ASSOC);
        ...
        if($info["ybdzt"] < 2){
            //更新状态
            $sfzh = dowith_sql($_SESSION["user"]);
            $sql = "UPDATE tdd SET `ybdzt` = '2' WHERE sfzh = '".$sfzh."';";
            $success = $lqcxk->exec($sql);
            if($success){
                $tips = "Success";
            } else {
        ...
        $sfzh = dowith_sql($_SESSION["user"]);
        $sql = "SELECT tzsbh FROM tdd WHERE sfzh = '".$sfzh."';";
        $rs = $lqcxk->query($sql);
        $info = $rs->fetch(PDO::FETCH_ASSOC);
        ...
        if($info["ybdzt"] < 3){
            //更新状态
            $sfzh = dowith_sql($_SESSION["user"]);
            $sql = "UPDATE tdd SET `ybdzt` = '3' WHERE sfzh = '".$sfzh."';";
            $success = $lqcxk->exec($sql);
            if($success){
                $tips = "Success";
            } else {
        ...
        if($info["ybdzt"] < 4){
            //更新状态
            $sfzh = dowith_sql($_SESSION["user"]);
            $sql = "UPDATE tdd SET `ybdzt` = '4' WHERE sfzh = '".$sfzh."';";
            $success = $lqcxk->exec($sql);
            if($success){
                $tips = "Success";
            } else {
        ...
        if($info["ybdzt"] < 5){
            //更新状态
            $sfzh = dowith_sql($_SESSION["user"]);
            $sql = "UPDATE tdd SET `ybdzt` = '5' WHERE sfzh = '".$sfzh."';";
            $success = $lqcxk->exec($sql);
            if($success){
                $tips = "Success";
            } else {
        ...
        $sfzh = dowith_sql($_SESSION["user"]);
        $sql = "SELECT xh, xm, xq, bj, bjjc FROM tdd WHERE sfzh = '".$sfzh."';";
        $rs = $lqcxk->query($sql);
        $info = $rs->fetch(PDO::FETCH_ASSOC);
        $res = $info;
        $tips = "Success";
